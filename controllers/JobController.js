...
exports.getJobs = async (req, res) => {
  const { keyword, location, category, type, minSalary, maxSalary, userId } =
    req.query;
  const query = {
    isClosed: false, // only fetch open jobs
  ...
  try {
    ...
    if (userId) {
      ...
      applications.forEach((app) => {
        applicationJobStatusMap[String(app.job)] = app.status;
      });
    }
    ...
    const jobsWithExtras = jobs.map((job) => {
      ...
      return {
        ...
        applicationStatus: applicationJobStatusMap[jobIdStr] || null,
      };
    });
